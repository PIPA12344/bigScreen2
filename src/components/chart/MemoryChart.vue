<template>
  <div id="mainChart6" style="height: 95%;width: 95%"></div>
</template>
...
export default {
  ...
  methods: {
    initChart() {
      this.chart = echarts.init(document.getElementById('mainChart6'), 'dark')
      this.chart.setOption(this.option)
      this.tidb.getTiKVMemory(this.triggerEvent)
    },
    triggerEvent() {
      ...
      let nameArr = []
      let seriesArr = []
      for(let i in this.tidb.tiKVMemoryData) {
        let arr = []
        nameArr.push(this.tidb.tiKVMemoryData[i].metric.instance)
        for(let j in this.tidb.tiKVMemoryData[i].values) {
          arr.push((this.tidb.tiKVMemoryData[i].values[j][1]/1024/1024/1024).toFixed(2))
        }
        seriesArr.push(arr)
      }
      for(let i in this.tidb.tiKVMemoryData[0].values) {
        timeArr.push(formatWithSeperator(new Date(this.tidb.tiKVMemoryData[0].values[i][0]*1000),'/',':'))
      }
